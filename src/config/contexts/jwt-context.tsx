...
const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    case ActionsJWT.SET: {
      const { jwt } = action
      if (jwt === undefined)
        throw new Error('in SET action jwt must be provided')
...
const useStateJWT = () => {
  ...
  if (context === undefined)
    throw new Error('useStateJWT must be within JwtProvider')
  return context
}
const useDispatchJWT = () => {
  const context = useContext(JwtDispatchContext)
  if (context === undefined)
    throw new Error('JwtDispatchJWT must be within JwtProvider')
  return context
}

export const useJwt = (): [State, Dispatch] => [useStateJWT(), useDispatchJWT()]
